...
class BorderMaker:
    ...
    def make_resized_picture(self, img: cv2, border_size: int = 0) -> np.ndarray:
    ...
    def get_correct_margin(self, canvas: cv2, img: cv2) -> Tuple[int, int]:
    ...
    def main(self, img_path: str, canvas_size: int = 1500, border_size: int = 0):
        try:
            ...
            resized_img = self.make_resized_picture(img, border_size)
            margin_x, margin_y = self.get_correct_margin(canvas, resized_img)
